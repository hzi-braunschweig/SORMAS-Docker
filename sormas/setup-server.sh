...
grep "^[[:blank:]]*<!-- <appender-ref ref="EMAIL_ERROR" /> -->[[:blank:]]*$" ${DOMAIN_DIR}/config/logback.xml
grep "^[[:blank:]]*<subject>SORMAS: %logger{20} - %m</subject>[[:blank:]]*$" ${DOMAIN_DIR}/config/logback.xml
